...
{
    // DatabaseConnection is an object containing methods
    // and properties for accessing a database, retrieving 
    // information and storing new records. It serves as a
    // wrapper for the built-in MySQL functions in PHP. 
	public $server		= null;
	...
	{
	    // Store the authentication information and
	...
	public function ping($message=null)
	{
		if (false === mysql_ping($this->link))
		{
			...
			if (!$this->connect())
			{
				...
				echo $this->link . "\n";
				logger("Throwing DB error", LEVEL_MINUTIA);
	...
	public function query($iQueryString=null, $exitOnError=true, $logMessage=true)
	{
		...
		if (function_exists('logger') && defined('LOG_LEVEL') && defined('LEVEL_DEBUG') && LOG_LEVEL == LEVEL_DEBUG && $logMessage)
		{
