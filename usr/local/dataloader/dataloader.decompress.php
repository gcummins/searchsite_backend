...
// (405) 869-0808
// Copyright (C) 2005 - 2007,  BATEA Business Solutions, Inc.
// -------------------------------------------------------------
...
// |                                                           |
// | File Created and modified: Aug, Sep, Nov 2006, Feb 2007,  |
// | Apr 2007                                                  |
...
/*
	...
	The script feedFieldMap.php will copy the tmp tables into a standard form in permanent table, then
	reindex the permanent tables using Sphinx.
	...
	Modification 2010-05-13
	Decompress all files when the arrive, rather than during the upload process. This is a major change.
		...
		if (mayUploadToDatabase($filename))
		{
			...
			if ($db->rowCount() == 0)
			{
				...
				$insertQuery = "INSERT INTO `".$dbDatabase."`.`filesitter`"
					. " (`original`, `time`, `filestart`, `size`, `modifiedtime`, `process`)"
					. " VALUES ('" . addslashes($filename) . "', $rightNow, $rightNow, '$fileSize', $filemTime, 0);";
...
function checkUploadComplete()
{
	...
	foreach($filelist as $file)
	{
		if (mayUploadToDatabase($file))
		{
			...
			if ($db->rowCount() == 0)
			{
				// TODO: Add the file to the filesitter table
			...
			elseif ($fsFile['process'] = 0)
			{
				// The field 'process' is set to something other than '0'
				// Check if an entry exists with a very old time. If so, reset process=0 and time=now
				$db->query("SELECT * FROM `$dbDatabase`.`filesitter`");				
			}

			// Get the file size
			logger("Getting the file size...", LEVEL_INFORMATION);
			$thisFileSize=getFileSize($upload.$file);
			
			// Get the file modification time
			logger("Getting the file modification time...", LEVEL_INFORMATION);
			$filemTime = filemtime($upload.$file);
			
			// Instead of comparing the MD5 sum, we will check the file size and mtime. --gcummins 080925
			//if(($fsFile['filestart']+$ul_wait_time)<time() && $fsFile['serial']==$md5Hash)
			if (($fsFile['filestart']+$ul_wait_time)<time() && $fsFile['size']==$thisFileSize)
			{
				...
				logger("Time exceeded and file sizes match. Filesitter process goes to \"" . getProcessName(1) . "\" on file: ".$file, LEVEL_STATUS);
	
				
				// Determine if file is compressed. If so, decomress it and return the new filename.
				$file = decompressFile($upload.$file);
			...
			//else if(($fsFile['filestart']+$ul_wait_time)<time() && $fsFile['serial']!=$md5Hash)
...
function decompressFile($path, $filename)
{
    // Determine if provided file is compressed. If it is, this function will decomress it, and return the new filename
...
function cleanFilesitter()
{
	...
	$db->query("SELECT `id`, `original` FROM `filesitter`");
	...
	switch ($removedFilesCounter)
	{
		case 0:
			logger("There are no orphaned files in the filesitter table.");
			break;
		case 1:
			logger("One orphaned record was removed from the filesitter table.");
			break;
		default:
			logger("$removedFilesCounter records were removed from the filesitter table.");
			break;
	}
}
...
function identifyFiles()
{
	...
	foreach ($db->arrays() as $fsQueue)
	{
		...
		// Close the file
		fclose($hndl);
		if (strstr($fsQueue['file'],"464063_524")) // Commission Junction filenames start with this string 
		{
			// COMMISSION JUNCTION
			logger("File: ".$fsQueue['file']." is from Commission Junction.  Filesitter process goes to \"" . getProcessname(3) . "\".", LEVEL_STATUS);
			$updateQuery = "UPDATE `$dbDatabase`.`filesitter` SET `feedfile`=1, `process`=3 WHERE `id` = ".$fsQueue['id'];
			$db->query($updateQuery);
			break;
		}
		else if ($firstline = "PROGRAMNAME      PROGRAMURL      CATALOGNAME     LASTUPDATED     NAME    KEYWORDS        DESCRIPTION     SKU     MANUFACTURER    MANUFACTURERID  UPC     ISBNCURRENCY    SALEPRICE       PRICE   RETAILPRICE     FROMPRICE       BUYURL  IMPRESSIONURL   IMAGEURL        ADVERTISERCATEGORY      THIRDPARTYID    THIRDPARTYCATEGORY      AUTHOR  ARTIST  TITLE   PUBLISHER       LABEL   FORMAT  SPECIAL GIFT    PROMOTIONALTEXT STARTDATE       ENDDATE OFFLINE ONLINE  INSTOCK CONDITION       WARRANTY        STANDARDSHIPPINGCOST")
        {
            // COMMISSION JUNCTION, version May 2010
            logger("File: " . $fsQueue['file'] . " is in the May 2010 Commission Junction format. Filesitter process goest to \"" . getProcessname(3) . "\".", LEVEL_STATUS);
            $updateQuery = "UPDATE `$dbDatabase`.`filesitter` SET `feedfile`=1, `process`=3 WHERE `id` = ".$fsQueue['id'];
            $db->query($updateQuery);
            break;
        }
		...
		else if(strstr($fsQueue['file'], "_276372_mp.txt"))
		{
			...
			$updateQuery = "UPDATE `".$dbDatabase."`.`filesitter` SET `feedfile`=2, `process`=3 WHERE `id` = ".$fsQueue['id'];
			$db->query($updateQuery);
			break;
		}
		else if (in_array(md5($firstline), $arrPerformicsMD5Sums))
		{
			...
			$updateQuery = "UPDATE `".$dbDatabase."`.`filesitter` SET `feedfile`=3, `process`=3 WHERE `id` = ".$fsQueue['id'];
			$db->query($updateQuery);
			break;
		}
		else
	...
	foreach ($db->arrays() as $fsList)
	{
		...
		if (DATALOADER_SAVE_COPIES_OF_FEEDS)
		{
				 ...
				 && mayStore(stripslashes($fsList['original'])))
			{
				...
				{
					// Copy the file to the backup location using PHP copy()
		...
		{
			logger("Not creating copies of feed files because of restriction in configuration settings.", LEVEL_INFORMATION);
		}
		...
		else if (strtolower(substr($upload.$fsList['original'], -4)) == '.zip')
		{
		     $workingFileIsCompressed = true;
		...
		if ($workingFileIsCompressed && $workingFileCompressionType == "gzip" && !$workingFileHasAppropriateExtension)
		{
	        ...
	        $db->query("UPDATE `$dbDatabase`.`filesitter` SET `original`='".addslashes($fsList['original']) . ".gz' WHERE `original`='" . addslashes($fsList['original']) . "' LIMIT 1");
    	    //Modify the existing variables to reflect the filename change
    	    logger("Updating \$fslist['original'] to add .gz extension.");
	        $fsList['original'] = $fsList['original'] . '.gz';
		}
		...
		{
			logger("Moved ".$upload.$fsList['original']." to ".$work.$fsList['original'], LEVEL_FILE_OPERATION);
			...
			if ($workingFileIsCompressed)
			{
                ...
                {
                    case "gzip":
        				logger("File: ".$work.$fsList['original']." is a gzip file. Gunzipping.", LEVEL_FILE_OPERATION);
        				$unzip_command_lastline = exec($gunzip_exec.'gunzip -f -q '.escapeshellarg(stripslashes($work.$fsList['original'])), $gunzip_command_ouput, $ret_var);
        				$fileSitterFile = preg_replace("/\.gz$/i", "", stripslashes($fsList['original']));
        				if($ret_var != '0')
        				{
        					logger("Gunzipping of '" . $work.$fsList['original'] . "' failed with error code: $ret_var.", LEVEL_DATA_WARNING);
        					logger("Full output from gunzip was : " . implode('---', $unzip_command_ouput), 2);
        					
        					logger("Removing: ".$fsList['original']." from filesitter.", LEVEL_DATABASE_OPERATION);
        					$deleteQuery = "DELETE FROM `".$dbDatabase."`.`filesitter` WHERE `id`='".$fsList['id']."'";
        					$db->query($deleteQuery);
        					
        					logger("Attempting to move file to the \"unrecognized\" directory.", LEVEL_FILE_OPERATION);
        					if(!rename(stripslashes($work.$fsList['original']), stripslashes($unrec.$fsList['original'])))
        					{
        						logger("Could not gunzip ".$work.$fsList['original']." or move file.", LEVEL_DATA_WARNING);
        					}
        				}
        				else
        				{
        				...
        				break;
                    case "zip":
                        logger("File: ".$work.$fsList['original']." is a ZIP file. Unzipping.", LEVEL_FILE_OPERATION);
                        $unzip_command_lastline = exec("/usr/local/bin/unzip -o " . escapeshellarg(stripslashes($work.$fsList['original'])) . " -d $work", $unzip_command_ouput, $ret_var);
                        $fileSitterFile = preg_replace("/\.zip$/i", ".txt", stripslashes($fsList['original']));
                        if ($ret_var != '0')
                        {
                            logger ("Unzipping of '" . $work.$fsList['original'] . "' failed with error code: $ret_var.", LEVEL_DATA_WARNING);
                            logger("Full output from unzip was : " . implode('---', $unzip_command_ouput), LEVEL_STATUS);
                            
                            // TODO: Remove the following line. It is for debugging only.
                            print_r($unzip_command_ouput);
        					...
        					if(!rename(stripslashes($work.$fsList['original']), stripslashes($unrec.$fsList['original'])))
        					{
        						logger("Could not unzip ".$work.$fsList['original']." or move file.", LEVEL_DATA_WARNING);
        					}
                        }
                        else
                        {
                            // Remove the original compressed file
                            unlink(escapeshellarg(stripslashes($work.$fsList['original'])));
			...
			{
				$fileSitterFile=$fsList['original'];
			...
			$updateQuery = "UPDATE `$dbDatabase`.`filesitter` SET `postsize`='$postSize', `file`='" . addslashes($fileSitterFile) . "', `storage`='". addslashes($fsList['original']) ."', `process`=2 WHERE `id`=".$fsList['id'];
			$db->query($updateQuery);
			
			logger("Changing filemode to 644 for: " . $work . $fileSitterFile, LEVEL_FILE_OPERATION);
			chmod($work.$fileSitterFile, 0644);
...
function cj_table($dbName, $tablename)
{
	logger("In " .__FUNCTION__ . "()", LEVEL_MINUTIA);
	
	// SQL to create a Commission Junction table
	$cjTable = "
...
function performics_table($dbName, $tablename)
{
	...
	/*$performicsTable = "
	  ...
	  `Interim_Description` varchar(255) NOT NULL default '',
	  ...
	  `Rew_Points` varchar(255) NOT NULL default '',
	  `ImageURL` varchar(255) NOT NULL default '',
	  `In_Stock` varchar(255) NOT NULL default '',
	  `Ship_Avail` varchar(255) NOT NULL default '',
	  `Ship_Cost` varchar(255) NOT NULL default '',
	  `Remove_Date` varchar(255) NOT NULL default '',
	  `Brand` varchar(255) NOT NULL default '',
	  `Partner_Specific` varchar(255) NOT NULL default '',
