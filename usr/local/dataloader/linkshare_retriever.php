...
/* Modification 2009-11-18
 *  This process no longer runs as a daemon, but as a single-run script. It must be called at regular intervals via cron 
 *  to retrieve Linkshare feed files via FTP.
 */

// Include the configuration settings file
include "config.inc.php";

define('LOG_FILE_NAME', 'linkshare_retriever.log');
...
$db->query("UPDATE `dl_daemon` SET `start`=$varstarttime, `wait`=0 WHERE `id`=$linkshareFeedId LIMIT 1");
...
if (is_array($buff) && count($buff))
{
	...
	else
	{
		...
		$numberOfFilesProcessed = $db->rowCount();
		if ($db->rowCount() != 0)
		{
		...
		$db->query("INSERT INTO `stats_linkshareretriever` (`files_processed`) VALUES ($numberOfFilesProcessed)");
	}
	unset($buff);
}
else
