...
define('DIR_GAL', '../images/');  // galleries base directory (relative path from
                            // spgm.php or the file requiring it if there's one)
define('DIR_LANG', 'spgm/lang/'); // language packs (relative path from spgm.php or
                             // the file requiring it if there's one)
define('DIR_THEMES', 'spgm/flavors/'); // themes base directory (relative path from
...
define('PARAM_NAME_PAGE', 'paging_spage');
...
define('CLASS_TABLE_WRAPPER', 'gallery_table');
...
{
	$task = htmlentities($_REQUEST['task']);
}
...
switch ($task)
{
	case 'deleteImage':
		dh_deleteImage();
	default:
		// Do nothing, and continue with script execution
		break;
}

function dh_deleteImage()
{
	global $module;
	...
	// Use this variable to determine whether the submission is valid and we should try to delete the image
	$validSubmission = true;
	
	// Gather the required fields
	if (isset($_REQUEST['delImage']) && !empty($_REQUEST['delImage']))
	{
		$imageToDelete = str_replace(array(".", "/", "\\"), "", urldecode($_REQUEST['delImage']));
	}
	else
	{
	...
	$absoluteFilePath = DEALHUNTING_GALLERY_FILESYSTEM_PATH . '/' . $imageToDelete;
	...
	if ($validSubmission)
	{
		...
		if (isset($_REQUEST['spgmPage']))
		{
			$spgmPageString = '&spgmPage=' . (int)$_REQUEST['spgmPage'];
		}
		else
		{
			$spgmPageString = '';
		...
		returnMessage(1101, sprintf(LANGUAGE_DELETE_OBJECT_NAME, 'image', $absoluteFilePath), false);
		returnToMainPage(getStartPage());
		return;
	}
	else
	{
		returnError(301, "Unable to delete file '$absoluteFilePath'", false);
		returnToMainPage(getStartPage());
		return;
	}
}
...
function spgm_LoadFlavor($strThemeName) {
  ...
  {
  	// The following line is to make the PHP strict debugger happy
  	define('GC_THEME_FILENAME_1', DIR_THEMES.$strThemeName);
  	include(GC_THEME_FILENAME_1.'/'.FILE_THEME);
...
function spgm_LoadLanguage($strCountryCode) {
  ...
  spgm_Trace(
	...
	if ($strCountryCode != '')
	{
		...
		{
			$gc_message = 'No support for lang. '.$strCountryCode.' &raquo; default: english (filename: ' . $filename_lang . ')<br />'; 
			spgm_Warning($gc_message);
		}
	}
}
    ...
    if ($bToBeDisplayed) {
      if (count($arrSubGalleries) > 0) {
        if ($spgm_cfg['conf']['gallerySortType'] == SORTTYPE_NAME) {
          ...
          usort($arrSubGalleries, 'spgm_CallbackCompareMTime');
    ...
    if ($bForDisplayPurpose) { 
      if (count($arrPictureFilenames) > 0) {
        if ($spgm_cfg['conf']['pictureSortType'] == SORTTYPE_NAME) {
          ...
          usort($arrPictureFilenames, 'spgm_CallbackCompareMTime');
...
{
	global $spgm_cfg, $module;

	include "includes/pagination.class.php";
	$paginator = new Pagination($module, ($iPageNumber*$spgm_cfg['conf']['thumbnailsPerPage']));
	$paginator->setRowsPerPage($spgm_cfg['conf']['thumbnailsPerPage']);
	echo $paginator->generateLinkBar();
	return;
...
function spgm_DisplayGalleryNavibar($strGalleryId, $strFilterFlags, $mixPictureId = '', $arrPictureFilenames)
{
  ...
  if (isset($arrPictureFilenames))
  {
  	unset($arrPictureFilenames);
  }
...
function spgm_DisplayPicture($strGalleryId, $iPictureId, $strFilterFlags) {
  ...
  $strPictureBasename = $strPictureFilename;  
  ...
  if ( spgm_IsPicture($strPictureFilename, $strGalleryId) ) {
    ...
    // Batea: disabled gallery header, since we will always display only one gallery
...
function spgm_DisplayThumbnails($strGalleryId, $arrPictureFilenames, $iPictureId, $iPageIndex, $strFilterFlags) {
  ...
  for ($i = $iPictureOffsetStart; $i < $iPictureOffsetStop; $i++) {
	...
	// BATEA: We don't want the "new" icon to ever be displayed
...
else
{
	...
	if (isPermitted('delete', $module))
	{
		$strHtmlNew = '<a href="#" onclick="return gallery_deleteImage(\'' . urlencode($strPictureFilename) . '\', ' . $iPageIndex . ', \''.ADMINPANEL_WEB_PATH.'\', '.$module.'); return false;" class="gallery_image_control"><div class="gallery_image_delete" style="width: ' . $arrPictureDim[0] . 'px;">&nbsp;</div></a>';
	}
	else
	{
		//$strHtmlNew = '<div class="gallery_image_delete" style="width: ' . $arrPictureDim[0] . 'px;">&nbsp;</div>';
		$strHtmlNew = '';
	}
...
;
	...
	else
	{
    	if (isPermitted('delete', $module))
		{	
			print '    <a href="#" onclick="gallery_deleteImage(\'' . urlencode($strPictureFilename) . '\', ' . $iPageIndex . ', \''.ADMINPANEL_WEB_PATH.'\', '.$module.'); return false;" class="">' . $strHtmlThumbnail . '</a>';
		}
		else
		{
    		print $strHtmlThumbnail;
...
/* Original Location of 'extract URL Parameters' */
// extract URL parameters
if ( ini_get('register_globals') == '1' )
{
	$spgm_cfg['global']['documentSelf'] = basename($PHP_SELF);
	$strParamGalleryId = DEALHUNTING_GALLERY_LOCATION; // hard-coded, because we offer only one gallery
...
else
{
	...
	if ( isset($_GET) )
	{
			foreach ($_GET as $key => $value)
		{
			...
			{
				if ($key != 'paging_spage') // We are using this instead of spgmPage to mark the current page
				{
...
/* End Original Location of 'extract URL Parameters' */
